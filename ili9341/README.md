...
* Also change `hdmi_cvt=480 320 60 6 0 0 0` to `hdmi_cvt=320 240 60 6 0 0 0`. If the first is left in place (correct it by adding the missing =), then after reboot the LCD should attempt to display a 480x320 and the hdmi monitor should also display the same image. If the 320x240 setting is used only the LCD will show an image after reboot.
```
